...
module.exports = class Point {
  ...
  set({ x = undefined, y = undefined, eventsEnabled = true }) {
    ...
    if (typeof x !== 'undefined' && this.isValid({ x: x })) {
      let newX =
        this._unit * Math.round((x - this._unitOffsetX) / this._unit) +
        this._unitOffsetX;
      if (this._x != newX) {
        this._x = newX;
        hasChanged = true;
    ...
    if (typeof y !== 'undefined' && this.isValid({ y: y })) {
      let newY =
        this._unit * Math.round((y - this._unitOffsetY) / this._unit) +
        this._unitOffsetY;
      if (this._y != newY) {
        this._y = newY;
        hasChanged = true;
    ...
    if (eventsEnabled && hasChanged && typeof this._onChanged === 'function')
      this._onChanged();
  }
  ...
  getX({ fromDesiredX }) {
    ...
    return (
      this._unit * Math.round((fromDesiredX - this._unitOffsetX) / this._unit) +
      this._unitOffsetX
    );
  }
  ...
  getY({ fromDesiredY }) {
    ...
    return (
      this._unit * Math.round((fromDesiredY - this._unitOffsetY) / this._unit) +
      this._unitOffsetY
    );
  }
  ...
  distanceTo({ point = undefined } = {}) {
    if (typeof point === 'undefined' || !(point instanceof Point))
      throw new SyntaxError('function expects at least one parameter');
    return new Size({
      width: Math.abs(point.x - this.x),
      height: Math.abs(point.y - this.y)
    });
  }
  ...
  clone() {
    return new Point({
      unit: this._unit,
      unitOffsetX: this._unitOffsetX,
      unitOffsetY: this._unitOffsetY,
      x: this._x,
      y: this._y
    });
  }
