...
module.exports = class Container extends Layer {
  ...
  set origin(value) {
    ...
    let dx =
      SymbolArt.scaling *
      Math.round((value.x - currentOrigin.x) / SymbolArt.scaling);
    let dy =
      SymbolArt.scaling *
      Math.round((value.y - currentOrigin.y) / SymbolArt.scaling);
  ...
  _sublayers = [];
  get sublayers() {
    return this._sublayers.slice(0);
  }
  ...
  add({ sublayer = undefined, atIndex = this._sublayers.length } = {}) {
    ...
    if (!(sublayer instanceof Layer))
      throw new TypeError(
        'sublayer expected a Layer but received: ' + sublayer
      );
    if (typeof atIndex !== 'number' || !Number.isInteger(atIndex))
      throw new TypeError(
        'atIndex expected an integer value but received: ' + typeof atIndex
      );
    ...
    if (sublayer.parent instanceof Container) {
      sublayer.parent.remove({
        sublayer: sublayer,
        deletesIfBecomesEmpty: this !== sublayer.parent
      });
    }
  ...
  remove({
    sublayer = undefined,
    sublayerWithUuidString = undefined,
    sublayerAtIndex = undefined,
    deletesIfBecomesEmpty = true
  } = {}) {
    if (
      typeof sublayer === 'undefined' &&
      typeof sublayerWithUuidString === 'undefined' &&
      typeof sublayerAtIndex === 'undefined'
    )
      throw new SyntaxError('function expects at least one parameter');
    let index = null;
    if (
      ...
      index = this.indexOf({
        sublayer: sublayer,
        sublayerWithUuidString: sublayerWithUuidString
      });
    ...
    if (typeof atIndex !== 'undefined') {
      ...
      if (atIndex < 0 || atIndex >= this._sublayers.length)
        throw new RangeError(
          'expected an index in range [0, ' +
            (this._sublayers.length - 1) +
            '] but received: ' +
            atIndex
        );
  ...
  indexOf({
    ...
    for (var index in this._sublayers) {
      ...
      if (sublayer instanceof Layer && layer === sublayer)
        ...
        typeof sublayerWithUuidString === 'string' &&
        layer.uuidString === sublayerWithUuidString
      )
        return parseInt(index);
      else if (
        recursive &&
  ...
  contains({ sublayer = undefined } = {}) {
    if (typeof sublayer === 'undefined')
      throw new SyntaxError('function expects at least one parameter');
    if (!(sublayer instanceof Symbol || sublayer instanceof Container))
      return false;
    let contains = false;
    this.depthFirstIterator(layer => {
  ...
  updateDepth() {
    let updatedDepth =
      Math.max(
        ...this._sublayers.map(a => (a instanceof Container ? a.depth : 0))
      ) + 1;
  ...
  isValid({ sublayer = undefined, name = undefined, parent = undefined } = {}) {
    ...
    if (sublayer instanceof Symbol) {
      ...
      valid =
        !this.contains({ sublayer: sublayer }) &&
  ...
  canInsert({ sublayer }) {
    ...
    if (layerDepth + 1 + this.distanceFromRoot > Container.maximumDepth)
      return false;
    let current = this;
    ...
    let symbolCount =
      sublayer instanceof Container ? sublayer.symbols.length : 1;
    if (currentSymbolCount + symbolCount > SymbolArt.maximumNumberOfSymbols)
      return false;
    let containerCount =
      sublayer instanceof Container ? 1 + sublayer.containers.length : 0;
    if (
      currentContainerCount + containerCount >
      SymbolArt.maximumNumberOfContainers
    )
      return false;
    return true;
  }
  ...
  clone({ retainUuid = true } = {}) {
    let clone = new Container({
      name: this.name,
      sublayers: this.sublayers.map(sublayer =>
        sublayer.clone({ retainUuid: retainUuid })
      )
    });
    if (retainUuid) {
