...
class SAMLFileUtils {
  static async parseIntoSymbolArt({ fileDataArrayBuffer }) {
    try {
      let rawFileContent = new Uint8Array(fileDataArrayBuffer).reduce(
      ...
      rawHelperImageContent =
        rawHelperImageContent === null ? null : rawHelperImageContent[0];
      ...
      if (window.DOMParser) {
        ...
        if (rawHelperImageContent) {
          helperImageXmlContent = parser.parseFromString(
            rawHelperImageContent,
            'text/xml'
          );
        }
      ...
      let symbolArt = await SAMLFileUtils._convertToSymbolArt({
        fileXmlContent: fileXmlContent.children[0],
        helperImageXmlContent: helperImageXmlContent
          ? helperImageXmlContent.children[0]
          : null
      });
      return symbolArt;
    } catch (e) {
  ...
  static async _convertToSymbolArt({ fileXmlContent, helperImageXmlContent }) {
    ...
    for (var index = fileXmlContent.children.length - 1; index >= 0; index--) {
      ...
      switch (xmlContent.nodeName) {
        case 'g':
          let layer = SAMLFileUtils._convertToContainer({
            fileXmlContent: xmlContent
          });
          if (layer instanceof Container) {
            ...
            while (layerStack.length > 0) {
              ...
              if (
                ...
                symbolArt.root.canInsert({ sublayer: item })
              ) {
          ...
          break;
        case 'layer':
          let symbol = SAMLFileUtils._convertToSymbol({
            fileXmlContent: xmlContent
          });
    ...
    if (
      helperImageXmlContent !== null &&
      helperImageXmlContent.nodeName === 'overlay-img'
    ) {
      let rawSource = null,
        version = 1;
      let positionX = 0,
      ...
      for (var key in helperImageXmlContent.attributes) {
        ...
        switch (name) {
          case 'version':
            version = Number.isSafeInteger(parseInt(value))
              ? parseInt(value)
              : 1;
            break;
          case 'src':
          ...
          case 'pos-x':
            ...
            positionX =
              (SymbolArt.viewableDimensions.width * (positionX - 960)) / 576;
            break;
          case 'pos-y':
            positionY = parseFloat(value);
            positionY =
              (-SymbolArt.viewableDimensions.height * (positionY - 480)) / 288;
            break;
          case 'scale':
      ...
      try {
        ...
        await symbolArt.helperImage.setImage({
          fromBase64EncodedString: rawSource
        });
        ...
        case 'g':
          ...
          if (layer instanceof Container) {
            ...
            while (layerStack.length > 0) {
              ...
              if (
                item instanceof Symbol ||
                container.canInsert({ sublayer: item })
              ) {
  ...
  static _convertToSymbol({ fileXmlContent }) {
    ...
    let origin = new Origin({
      x:
        SymbolArt.scaling *
        0.25 *
        [0, 'ltx', 'lbx', 'rtx', 'rbx'].reduce(
          (a, b) => a + parseInt(fileXmlContent.attributes[b].nodeValue)
        ),
      y:
        -SymbolArt.scaling *
        0.25 *
        [0, 'lty', 'lby', 'rty', 'rby'].reduce(
          (a, b) => a + parseInt(fileXmlContent.attributes[b].nodeValue)
        )
    });
    ...
    symbol.frame.vertexA.set({
      x:
        SymbolArt.scaling *
          parseInt(fileXmlContent.attributes['ltx'].nodeValue) -
        origin.x,
      y:
        -SymbolArt.scaling *
          parseInt(fileXmlContent.attributes['lty'].nodeValue) -
        origin.y
    });
    symbol.frame.vertexB.set({
      x:
        SymbolArt.scaling *
          parseInt(fileXmlContent.attributes['rtx'].nodeValue) -
        origin.x,
      y:
        -SymbolArt.scaling *
          parseInt(fileXmlContent.attributes['rty'].nodeValue) -
        origin.y
    });
    for (var key in fileXmlContent.attributes) {
      let name = fileXmlContent.attributes[key].nodeName;
      ...
      switch (name) {
        ...
        case 'color':
          symbol.color = new Color({
            hexValue: parseInt('0x' + value.replace('#', ''))
          });
          break;
        case 'alpha':
          switch (parseFloat(value)) {
            ...
            case 225: // Backward-compatibility
              symbol.opacity.index = 0;
              break;
            case 0.247059:
            case 193:
              symbol.opacity.index = 1;
              break;
            case 0.372549:
            case 161:
              symbol.opacity.index = 2;
              break;
            case 0.498039:
            case 129:
              symbol.opacity.index = 3;
              break;
            case 0.623529:
            case 97:
              symbol.opacity.index = 4;
              break;
            case 0.74902:
            case 65:
              symbol.opacity.index = 5;
              break;
            case 0.87451:
            case 33:
              symbol.opacity.index = 6;
              break;
            case 1:
              symbol.opacity.index = 7;
              break;
          }
          break;
        default:
  ...
  static exportAsBlob({ symbolArt }) {
    let xmlContent = SAMLFileUtils._convertFromSymbolArt({
      symbolArt: symbolArt
    });
  ...
  static _convertFromSymbolArt({ symbolArt }) {
    ...
    if (symbolArt.helperImage.imageData) {
      content += `<overlay-img src="${
        symbolArt.helperImage.imageData
      }" pos-x="${
        (symbolArt.helperImage.positionX * 576) /
          SymbolArt.viewableDimensions.width +
        960
      }" pos-y="${
        (symbolArt.helperImage.positionY * 288) /
          -SymbolArt.viewableDimensions.height +
        480
      }" scale="${
        (symbolArt.helperImage.scaleX * 3) / SymbolArt.scaling
      }" rot="${-symbolArt.helperImage.rotationAngle}" alpha="${
        symbolArt.helperImage.opacity
      }" green-screen="${
        symbolArt.helperImage.greenScreenEnabled ? 'true' : 'false'
      }"/>`;
    }
    content += `<sa name="${symbolArt.root.name}" visible="${
      symbolArt.root.isHidden ? 'false' : 'true'
    }" version="1" author="${symbolArt.authorId}" width="${
      symbolArt.type.width
    }" height="${symbolArt.type.height}" sound="${
      symbolArt.soundOption.index
    }">`;
  ...
  static _convertFromContainer({ container }) {
    ...
    let content = `<g name="${container.name}" visible="${
      container.isHidden ? 'false' : 'true'
    }">`;
  ...
  static _convertFromSymbol({ symbol }) {
    ...
    let topLeftY = Math.round(
      (frame.vertexA.y + origin.y) / -SymbolArt.scaling
    );
    let topRightX = Math.round(
      (frame.vertexB.x + origin.x) / SymbolArt.scaling
    );
    let topRightY = Math.round(
      (frame.vertexB.y + origin.y) / -SymbolArt.scaling
    );
    let bottomRightX = Math.round(
      (frame.vertexC.x + origin.x) / SymbolArt.scaling
    );
    let bottomRightY = Math.round(
      (frame.vertexC.y + origin.y) / -SymbolArt.scaling
    );
    let bottomLeftX = Math.round(
      (frame.vertexD.x + origin.x) / SymbolArt.scaling
    );
    let bottomLeftY = Math.round(
      (frame.vertexD.y + origin.y) / -SymbolArt.scaling
    );
    let alpha = 0;
    ...
    return `<layer name="${symbol.name}" visible="${
      symbol.isHidden ? 'false' : 'true'
    }" type="${symbol.asset.index}" color="#${
      symbol.color.hex
    }" alpha="${alpha}" ltx="${topLeftX}" lty="${topLeftY}" lbx="${bottomLeftX}" lby="${bottomLeftY}" rtx="${topRightX}" rty="${topRightY}" rbx="${bottomRightX}" rby="${bottomRightY}"/>`;
  }
}
